	 	...
	 	if(!cur_count)
	 	{
	        ...
	        if( FD_ISSET(Stdin, &readSet) ){
	            ...
	            if(lenghtRead){
	               ...
	               // std::cout<<"lenght="<<lenghtRead<<" sebuf |"<<sendBuffer<<"|\n";
	            }
	            cur_count++;
	        }
   	    }
        	...
        	if(cur_count%3==0){
        		if( write(sock, "/msg random hi dude\n", 20) < 0){
                    std::cout << "[Disconnected from server]" << std::endl;
                    stop = true;
                }
        	}
      	else if(cur_count%5==0){
        		char a[11]="/add ch";
        		a[7]='0'+cur_count%10 , a[8]='\n';

        		if( write(sock, a, std::strlen(a)) < 0){
                    std::cout << "[Disconnected from server]" << std::endl;
                    stop = true;
                }
        	}
        	else if( write(sock, "/all hi everyone\n", 17) < 0){
                    std::cout << "[Disconnected from server]" << std::endl;
                    stop = true;
                }
        cur_count++;
